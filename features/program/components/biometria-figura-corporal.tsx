...
export function BiometriaFiguraCorporal({
  ...
  const [wrapperW, setWrapperW] = useState(200);
  useEffect(() => {
    const el = wrapperRef.current;
    if (!el) return;
    const update = () => setWrapperW(el.clientWidth || 200);
    update();
    const ro =
  ...
  const handleMouseOver = useCallback(
    (e: React.MouseEvent) => {
      ...
      const id =
        target.getAttribute("id") || target.closest("[id]")?.getAttribute("id");
  ...
  const highlight: { stroke: string; strokeWidth: number } | undefined =
    activeZone ? { stroke: "var(--primary)", strokeWidth: 1.5 } : undefined;
  ...
  data.push({
    slug: "abs",
    color: aColor,
    ...(activeZone === "cintura" ? { styles: highlight } : {}),
  });
  data.push({
    slug: "obliques",
    color: aColor,
  ...
  if (hColor !== "#D1D5DB")
    data.push({
      slug: "gluteal",
      color: hColor,
      ...(activeZone === "cadera" ? { styles: highlight } : {}),
    });
  else if (activeZone === "cadera")
    data.push({ slug: "gluteal", styles: highlight });
  if (activeZone === "cadera")
    data.push({ slug: "quadriceps", styles: highlight });
  ...
  if (pctGrasa != null || imc != null) {
    if (bColor !== "#D1D5DB")
      data.push({
        slug: "chest",
        color: bColor,
        ...(activeZone === "torso" ? { styles: highlight } : {}),
      });
    else if (activeZone === "torso")
      data.push({ slug: "chest", color: "#D1D5DB", styles: highlight });
  } else if (activeZone === "torso") {
    data.push({ slug: "chest", styles: highlight });
  }

  // Forearm (muñeca) — always present for hover highlight even if no distinct fill
  if (activeZone === "muneca")
    data.push({ slug: "forearm", styles: highlight });
  ...
  if (activeZone === "talla") {
    for (const slug of TALLA_SLUGS) {
      ...
      if (!data.some((d) => d.slug === slug))
        data.push({
          slug: slug as (typeof data)[number]["slug"],
          styles: highlight,
        });
    }
  }

  const tooltip = activeZone
    ? buildTooltipContent(activeZone, exacta, gender, iccAlto)
    : null;
              ...
              {tooltip.rows.map((r) => (
                ...
                >
                  ...
                  <span className="flex items-center gap-1.5">
                    ...
                    {r.category && (
                      <Badge
                        className={`border text-[10px] ${categoryTone(r.category)}`}
                      >
                        {r.category}
                      </Badge>
                    )}
...
function buildTooltipContent(
  ...
  title: string;
  rows: { label: string; value: string; category: string | null }[];
} | null {
      ...
      rows: [
        {
          label: "% Grasa",
          value:
            exacta.pct_grasa != null ? `${exacta.pct_grasa.toFixed(1)}%` : "—",
          category: grasaCat,
        },
        {
          label: "% Magra",
          value:
            exacta.pct_magra != null ? `${exacta.pct_magra.toFixed(1)}%` : "—",
          category: null,
        },
      ],
    };
