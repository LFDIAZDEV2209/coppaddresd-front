...
/** Select nativo estilizado: borde visible, flecha custom, mismo alto que los inputs. */
  ...
  return (
    <div className="relative w-full">
      <select
        ...
        onChange={(e) => onChange(e.target.value)}
        className={cn(
          "h-9 w-full cursor-pointer appearance-none rounded-lg border border-border bg-white pr-8 pl-3 text-[13px] font-medium text-foreground shadow-xs outline-none transition-all hover:border-primary/50 hover:bg-muted/30 focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/40 aria-[invalid=true]:border-destructive/60 aria-[invalid=true]:bg-destructive-soft/20 aria-[invalid=true]:text-destructive",
        )}
        aria-invalid={invalid}
      >
        {options.map((option) => (
