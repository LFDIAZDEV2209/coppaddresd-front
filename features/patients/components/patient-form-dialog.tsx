...
export function PatientFormDialog({
  ...
  const [form, setForm] = useState<PatientInput>(() =>
    patient ? { ...patient } : emptyForm,
  );
  const [validationError, setValidationError] = useState<string | null>(null);

  const update = <K extends keyof PatientInput>(
    field: K,
      ...
      <DialogContent className="max-h-[92vh] max-w-2xl overflow-y-auto p-0">
        <DialogHeader className="border-b border-border bg-primary-soft px-6 py-5">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col gap-1">
              ...
              <DialogDescription>
                Registra la información clínica y de contacto.
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        <form onSubmit={submit} className="flex flex-col gap-5 px-6 py-5">
              ...
              <Field label="Tipo de documento" required>
                <select
                  ...
                  onChange={(event) =>
                    update(
                      "documentType",
                      event.target.value as PatientInput["documentType"],
                    )
                  }
                >
              ...
              <Field label="Género" required>
                <select
                  ...
                  onChange={(event) =>
                    update(
                      "gender",
                      event.target.value as PatientInput["gender"],
                    )
                  }
                >
          ...
          <Field label="Observaciones">
            <textarea
              className="min-h-20 w-full resize-y rounded-md border border-input bg-background px-3 py-2 text-sm outline-none placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-ring"
              value={form.notes}
