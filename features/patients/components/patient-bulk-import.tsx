...
import {
  downloadPatientsTemplateCsv,
  parsePatientsCsv,
  validatePatientRows,
  type BulkPatientPreview,
  type BulkPatientRow,
} from "../services/patients-csv";
import {
  createBulkPatients,
...
/**
 * Importación masiva de pacientes conectada al endpoint real.
 * Flujo: plantilla CSV → preview editable con revalidación → confirmación
 * (con selector de clínica) → procesamiento (POST /api/v1/patients/bulk,
 * request único con spinner indeterminado) → resultado con mapeo de
 * results[] por line.
 *
 * Resolución de clínica: clínica activa del contexto de la app si está
 * disponible; si no, la primera clínica del árbol organizacional.
 */
export function PatientBulkImport() {
  const t = useT();
  const router = useRouter();
  const { activeClinicId } = useAppContext();
  const [stage, setStage] = useState<Stage>("upload");
  const [rows, setRows] = useState<BulkPatientRow[]>([]);
  const [preview, setPreview] = useState<BulkPatientPreview | null>(null);
  const [result, setResult] = useState<BulkCreatePatientsResult | null>(null);
  const [fileError, setFileError] = useState<string | null>(null);
  const [fileName, setFileName] = useState<string | null>(null);
  const [dragOver, setDragOver] = useState(false);
  // Inicializar con la clínica activa del contexto si existe.
  const [clinicId, setClinicId] = useState<string | null>(activeClinicId);
  const [clinics, setClinics] = useState<OrgClinic[]>([]);
  ...
  // Resolución de clínica: si no hay clínica activa del contexto, buscar la
  // primera del árbol organizacional.
  useEffect(() => {
  ...
  const revalidate = useCallback(
    (next: BulkPatientRow[]) => {
      ...
      const summary = validatePatientRows(copies, { clinicId });
      setRows(copies);
      setPreview(summary);
    },
    [clinicId],
  ...
  const updateRow = (index: number, patch: Partial<BulkPatientRow>) => {
    revalidate(
  ...
  const startImport = async () => {
    ...
    try {
      const response = await createBulkPatients({
  ...
  return (
    <div className="flex flex-col gap-6 p-4 sm:p-6">
      <PageHeader
        title={t("Crear pacientes masivamente")}
        description={t(
          "Importá un archivo CSV con la plantilla: revisá la validación y creá todos los pacientes de una vez.",
        )}
        icon={Stethoscope}
        actions={
          <Button
      ...
      {stage === "upload" && (
        <div className="animate-slide-up flex flex-col gap-4 rounded-2xl border border-border bg-card p-5 sm:p-8">
          ...
          <div className="flex flex-wrap items-center gap-2.5 border-t border-border/60 pt-4">
            <Button variant="outline" onClick={downloadPatientsTemplateCsv}>
        ...
        <div className="animate-slide-up flex flex-col gap-4 rounded-2xl border border-border bg-card p-5">
          <div className="flex flex-wrap items-center gap-2">
            <SummaryChip
              tone="success"
              icon={CheckCircle2}
              label={`${preview.valid} ${t("pacientes válidos")}`}
            />
            {preview.invalid > 0 && (
              <SummaryChip
                tone="warning"
            ...
            {preview.duplicates > 0 && (
              <SummaryChip
                tone="destructive"
                icon={XCircle}
                label={`${preview.duplicates} ${t("duplicados")}`}
              />
            )}
                      ...
                      <td className="px-2 py-1.5">
                        <EditableCell
                          ...
                          invalid={row.errors.some(
                            (e) =>
                              e.includes("Documento") && !e.includes("duplicado"),
                          )}
                          onChange={(value) =>
                            updateRow(index, { documentNumber: value })
                          }
                        />
                      </td>
                      <td className="px-2 py-1.5">
                        <EditableCell
                          value={row.email}
                          placeholder={t("email@dominio.com (opcional)")}
                          invalid={row.errors.some(
                            (e) =>
      ...
      {stage === "confirm" && preview && (
        <div className="animate-scale-in flex flex-col items-center gap-4 rounded-2xl border border-border bg-card px-6 py-12 text-center">
          ...
          <h2 className="text-lg font-bold text-foreground">
            {t("¿Crear {count} pacientes?", {
              count: String(preview.valid),
            })}
          </h2>
          <p className="max-w-md text-[13px] text-muted-foreground">
            {t(
              "Se creará el perfil de cada paciente válido. Las filas con errores se omitirán y podrás corregirlas después.",
            )}
          </p>

          {/* Selector de clínica en etapa de confirmación */}
          {clinics.length > 1 && (
            <div className="flex w-full max-w-xs flex-col items-start gap-1.5 text-left">
              ...
              <NativeSelect
                value={clinicId ?? ""}
                onChange={(value) => setClinicId(value || null)}
                options={[
