      ...
      <div className="flex flex-col gap-3 p-3">
        ...
        <div className="flex items-start justify-between gap-3">
            ...
            {member ? (
              ...
              <>
                <span className="flex size-9 shrink-0 items-center justify-center rounded-full bg-muted text-xs font-bold text-muted-foreground">
                  {profileName(
                    rp.post.profile?.displayName ?? "",
                    rp.post.profile?.isSystem,
                    t,
                  )
                    .slice(0, 2)
                    .toUpperCase()}
          ...
          <StatusBadge
            ...
            color={{
              bg: "var(--destructive-soft, #fee2e2)",
              text: "var(--destructive, #ef4444)",
              dot: "var(--destructive, #ef4444)",
            }}
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <span className="ml-auto flex items-center gap-3 text-[11px] text-muted-foreground">
            <span className="flex items-center gap-1">
              <Heart className="size-3" /> {rp.post.likes?.length ?? 0}
            </span>
            <span className="flex items-center gap-1">
              <MessageCircle className="size-3" />{" "}
              {rp.post.comments?.length ?? 0}
            </span>
            <span className="flex items-center gap-1">
              <Eye className="size-3" /> {rp.post.viewCount}
            </span>
          </span>
        </div>
        ...
        <div className="flex flex-col gap-1.5 rounded-lg border border-destructive/20 bg-destructive/5 p-2">
          ...
          {rp.reports.map((r) => (
            <div
              key={r.id}
              className="flex flex-col gap-0.5 rounded bg-background/60 px-2 py-1.5 text-xs"
            >
            ...
            {rp.post.profile?.id && !rp.post.profile?.isSystem && (
              <Button
                ...
                onClick={() =>
                  onBan(
                    rp.post.profile!.id,
                    profileName(rp.post.profile?.displayName ?? "", false, t),
                    isBanned,
                  )
                }
              >
    ...
    <div className="overflow-hidden rounded-xl border border-border bg-card transition-all hover:shadow-md hover:shadow-black/5">
      <div className="flex flex-col gap-3 p-3">
            ...
            {member ? (
              ...
              <>
                ...
                <div className="flex min-w-0 flex-col gap-0.5">
                  <span className="truncate text-sm font-semibold">
                    {profileName(
                      rc.comment.profile?.displayName ?? "",
                      false,
                      t,
                    )}
                  </span>
                  <span className="truncate text-xs text-muted-foreground">
                    {rc.comment.createdAt}
                  </span>
                </div>
              </>
            )}
        ...
        <div className="rounded-lg border border-border bg-muted/30 p-2 text-xs text-muted-foreground">
          ...
          {rc.post.body?.slice(0, 80)}
          {rc.post.body && rc.post.body.length > 80 ? "..." : ""}
        </div>
...
export function ModerationPage() {
  ...
  const [confirmResolveCommentReportId, setConfirmResolveCommentReportId] =
  ...
  const handleBan = () => {
    ...
    banProfile(
      banTarget.id,
      banReason || t("Incumplimiento de normas de la comunidad"),
    );
    setBanTarget(null);
    setBanReason("");
  };
    ...
    <div className="flex flex-col gap-6 p-4 sm:p-6">
      ...
      <div className="flex items-center gap-2.5 rounded-xl border border-[#991B1B] bg-destructive-soft px-3 py-2.5">
        ...
        <div className="flex flex-col gap-0 min-w-0">
          <p className="text-[13px] font-semibold leading-tight text-[#991B1B]">
            {t("Reportes activos")}
          </p>
      ...
      {canModerate && (
        <div className="flex flex-col gap-3 overflow-hidden rounded-2xl border border-border bg-card p-4">
          ...
          {!searchProfilesLoading &&
            memberSearch &&
            searchProfilesResult.length === 0 && (
              <p className="py-2 text-center text-xs text-muted-foreground">
                {t("No se encontraron miembros")}
              </p>
            )}
              ...
              {searchProfilesResult.map((p) => {
                ...
                return (
                  ...
                  >
                    <div className="flex min-w-0 items-center gap-3">
                      <span className="flex size-9 shrink-0 items-center justify-center rounded-full bg-muted text-xs font-bold text-muted-foreground">
                        {profileName(p.displayName, p.isSystem, t)
                          .slice(0, 2)
                          .toUpperCase()}
                      </span>
                      <div className="flex min-w-0 flex-col gap-0.5">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (isBanned) {
                          ...
                          setBanTarget({
                            id: p.id,
                            name: profileName(p.displayName, false, t),
                          });
                        }
                      }}
                    >
            ...
            reportedPosts.length === 0 && (
              <div className="flex flex-col items-center gap-3 rounded-xl border border-border bg-card py-12 text-center">
                ...
                <p className="text-sm text-muted-foreground">
                  {t("Sin reportes de publicaciones")}
                </p>
      ...
      {activeTab === "comments" && (
        <>
          ...
          {reportedCommentsError && (
            <div className="rounded-xl border border-destructive/20 bg-destructive/5 p-4 text-center text-sm text-destructive">
              {t(
                "No se pudieron cargar los reportes de comentarios. Intenta de nuevo.",
              )}
            </div>
          )}
          ...
          {!reportedCommentsLoading &&
            ...
            reportedComments.length === 0 && (
              <div className="flex flex-col items-center gap-3 rounded-xl border border-border bg-card py-12 text-center">
                <CheckCircle className="size-10 text-success" />
                <p className="text-sm text-muted-foreground">
                  {t("Sin reportes de comentarios")}
                </p>
                <p className="text-xs text-muted-foreground">
                  {t("No hay comentarios reportados pendientes.")}
                </p>
              </div>
            )}
      ...
      <AlertDialog
        ...
        onOpenChange={(o) => {
          if (!o) setConfirmDeletePost(null);
        }}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t(
                "¿Estás seguro de que deseas eliminar esta publicación reportada? Esta acción la ocultará de la comunidad y no se podrá deshacer.",
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t(
                "¿Confirmas que este reporte cumple con las normas? Se eliminará el reporte y la publicación permanecerá visible en la comunidad.",
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {banTarget
                ? t(
                    "Se restringirá el uso de la comunidad para {name}. No podrá publicar ni comentar, pero seguirá teniendo acceso a la plataforma.",
                    { name: banTarget.name },
                  )
                : ""}
            </DialogDescription>
          ...
          <div className="flex flex-col gap-3 py-2">
            <div className="flex flex-col gap-1.5">
              ...
              <Textarea
                ...
                placeholder={t(
                  "Ej: Contenido que incumple las normas de la comunidad",
                )}
                rows={3}
              />
            </div>
            <p className="text-[11px] text-muted-foreground">
              {t(
                "Esta acción solo restringe el acceso a la comunidad. Podrás restaurarlo en cualquier momento.",
              )}
            </p>
          </div>
          <DialogFooter>
      ...
      <AlertDialog
        ...
        onOpenChange={(o) => {
          if (!o) setConfirmDeleteComment(null);
        }}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t(
                "¿Estás seguro de que deseas eliminar este comentario reportado? Esta acción lo ocultará de la comunidad y no se podrá deshacer.",
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog
        open={Boolean(confirmResolveCommentReportId)}
        onOpenChange={(o) => {
          if (!o) setConfirmResolveCommentReportId(null);
        }}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t(
                "¿Confirmas que este reporte cumple con las normas? Se eliminará el reporte y el comentario permanecerá visible.",
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => {
                if (confirmResolveCommentReportId)
                  handleResolveCommentReport(confirmResolveCommentReportId);
              }}
            >
              <CheckCircle data-icon="inline-start" className="size-3.5" />
              {t("Confirmar y resolver")}
