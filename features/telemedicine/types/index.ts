/**
 * Tipos del módulo de Telemedicina (espejo de los DTOs y enums del microservicio
 * CoppAddresd.Telemedicine y de los catálogos del backend que la UI consume).
 */
...
export interface TelemedicineRequestDto {
  id: string;
  patientId: string;
...
export interface PaginatedAdminAppointmentsResult {
  items: TelemedicineAppointmentDto[];
  total: number;
  page: number;
  pageSize: number;
  totalPages: number;
...
export interface TelemedicineAlertDto {
  id: string;
  type: AlertType;
