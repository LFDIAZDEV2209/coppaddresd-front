...
/**
 * Router del dashboard de Telemedicina por capacidades (extensible para roles
...
export function TelemedicineDashboard() {
  ...
  const view = useMemo(() => {
    if (can("Telemedicine.AdminView")) return "admin";
 ...
 * accesos a los módulos de telemedicina que su rol permite. Cada enlace se
 * muestra según el permiso del contexto; agregar un módulo futuro = una
 * entrada más (declarativo, sin condicionales por rol).
 */
function StaffDashboard() {
  const { can } = useAppContext();

  const links = [
    can("Telemedicine.AgendaView") && {
      href: "/telemedicine/agenda",
      icon: CalendarDays,
      label: "Agenda",
      description: "Agenda de los profesionales",
    },
    can("Telemedicine.RequestsView") && {
      href: "/telemedicine/solicitudes",
      icon: Inbox,
      label: "Solicitudes",
      description: "Solicitudes de los pacientes",
    },
    can("Telemedicine.AlertsView") && {
      href: "/telemedicine/alertas",
      icon: Bell,
      label: "Alertas",
      description: "Bandeja de notificaciones",
    },
    can("Telemedicine.AppointmentsView") && {
      href: "/telemedicine/citas",
      icon: ClipboardList,
      label: "Citas",
      description: "Citas de telemedicina",
    },
