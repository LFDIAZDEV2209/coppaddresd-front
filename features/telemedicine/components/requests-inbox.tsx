...
import { confirmRequest } from "../services/telemedicine-service";
import {
  formatDate,
  formatDateTime,
  requestStatusColor,
 ...
 * con icono, stats cards, filtros (estado server-side + búsqueda local),
 * agrupación por día (Hoy/Ayer/fecha) y acción de confirmación para el
...
export function RequestsInbox({
  ...
  const [confirming, setConfirming] = useState<TelemedicineRequestDto | null>(
    null,
  );
  ...
  const groups = useMemo(() => {
    ...
    const result: Array<{
      label: string;
      requests: TelemedicineRequestDto[];
    }> = [];
    ...
    <div className="flex flex-col gap-6 p-6">
      <PageHeader
        ...
        description={
          scope === "professional"
            ? professionalName
              ? `${pending} pendientes por confirmar · asignadas a ${professionalName}`
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard
          ...
          variant="primary"
          context={
            hasActiveFilters ? "según filtros activos" : "todas las solicitudes"
          }
        />
        <StatCard
          label="Convertidas"
...
function RequestItem({
  ...
  request: TelemedicineRequestDto;
  canConfirm: boolean;
  onConfirm: (request: TelemedicineRequestDto) => void;
}) {
