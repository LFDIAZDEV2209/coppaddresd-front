        ...
        {!professionalId ? (
          ...
          <div className="grid grid-cols-7 gap-1">
            ...
            {days.map((day) => {
              ...
              const agendaHref = `/telemedicine/agenda?fecha=${toDateKey(day)}`;
              return (
                <div
                  key={key}
                  className={
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col gap-1">
                    ...
                    {overflow > 0 && (
                      <Link
                        href={`/telemedicine/agenda?fecha=${toDateKey(day)}`}
                        className="rounded-md px-1.5 py-0.5 text-[10px] font-semibold text-primary-strong transition-colors hover:bg-primary-soft hover:text-primary-strong"
                      >
                        +{overflow} más
                      </Link>
...
function DayDetailDialog({
  ...
  date: Date;
  appointments: TelemedicineAppointmentDto[];
  onClose: () => void;
}) {
  const today = new Date();
  const isToday = date.toDateString() === today.toDateString();
  const agendaHref = `/telemedicine/agenda?fecha=${toDateKey(date)}`;
  return (
    <Dialog
      open
      onOpenChange={(open) => {
