...
import { confirmRequest, fetchAdminRequests } from "../services/telemedicine-service";
import {
  requestStatusColor,
  requestStatusLabel,
  formatDate,
...
import type { TelemedicineRequestDto } from "../types";
...
export function ProfessionalRequests() {
  ...
  const {
    ...
    error,
    refetch,
  } = useAdminList<TelemedicineRequestDto>(
    professionalId
  ...
  const [confirming, setConfirming] = useState<TelemedicineRequestDto | null>(null);
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      <PageHeader
        title="Solicitudes"
        description={professional ? `Solicitudes asignadas a ${professional.fullName}` : "Solicitudes de telemedicina"}
        icon={Inbox}
      />
