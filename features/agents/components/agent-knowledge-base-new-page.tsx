...
/**
 * Página dedicada de creación de knowledge bases por agente
 * (/agents/[id]/knowledge/new): reemplaza al antiguo modal "Nueva knowledge
 * base" del detalle del agente. Scope fijo: específico del agente.
 */
export function AgentKnowledgeBaseNewPage({ agentTypeId }: { agentTypeId: string }) {
  const t = useT();
  ...
  const submit = async (values: { name: string; description: string | null }) => {
    ...
    try {
      ...
      // Feedback de éxito + regreso al detalle del agente (tab conocimiento).
      router.push(`/agents/${agentTypeId}?creado=1&tab=knowledge`);
    } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      <PageHeader
        title={t('Nueva knowledge base')}
        description={t('La base queda disponible para este agente.')}
        icon={BookOpen}
      />
