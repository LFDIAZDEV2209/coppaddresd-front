...
export function MonitoringPageContent() {
  ...
  const tokensTotal = monitoring.executions.reduce(
    (acc, e) => acc + (e.tokensIn ?? 0) + (e.tokensOut ?? 0),
    0,
  );
  ...
  const avgLatency =
    monitoring.executions.length > 0
      ...
      : null;
  const withFeedback = monitoring.executions.filter((e) => e.feedbackRating !== null).length;
    ...
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Select
          ...
          onValueChange={(value) =>
            monitoring.setFilters({ status: value === "all" ? "" : (value ?? "") })
          }
        >
              ...
              {monitoring.executions.map((execution) => (
                <TableRow key={execution.id}>
                  ...
                  <TableCell className="font-mono text-[11.5px] text-muted-foreground">
                    {execution.agentTypeId.slice(0, 8)}
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-[12.5px]">{execution.model ?? "—"}</TableCell>
                  <TableCell className="text-[12.5px]">
                    {execution.latencyMs !== null
                      ? `${execution.latencyMs.toLocaleString("es-ES")} ms`
                      : "—"}
                  </TableCell>
                  <TableCell className="text-[12.5px]">
