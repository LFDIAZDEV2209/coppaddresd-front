    ...
    <div className="flex flex-col gap-6 p-4 sm:p-6">
      ...
      <div className="rounded-2xl border border-border bg-card p-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold">{t('Vista previa de la tienda')}</p>
            <p className="text-xs text-muted-foreground">
              {stats ? `${stats.visible} ${t('productos visibles')}` : "..."} · {t('vista de catálogo móvil')}
            </p>
          </div>
        </div>
      </div>
          ...
          <section className="flex flex-col gap-3">
            ...
            {rest.length ? (
              ...
              <div className="rounded-2xl border border-dashed border-border bg-card py-16 text-center">
                ...
                <p className="mt-2 text-sm font-semibold">{t('No hay productos visibles')}</p>
                <p className="text-xs text-muted-foreground">
                  {t('Publica productos desde la gestión de tienda.')}
                </p>
              </div>
            )}
          </section>
    ...
    <div className="group flex flex-col overflow-hidden rounded-2xl border border-border bg-card transition-all hover:shadow-lg hover:shadow-black/5">
      ...
      <div className="flex flex-1 flex-col gap-1.5 p-4">
        ...
        <div className="mt-auto flex items-end justify-between pt-2">
          ...
          >
            {item.stock > 0 ? `${item.stock} ${t('en stock')}` : t('Agotado')}
          </span>
        </div>
      </div>
    </div>
