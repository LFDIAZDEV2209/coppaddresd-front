...
import { Copy, Loader2, MailPlus, Search, Stethoscope, UserRound } from "lucide-react";
...
export default function EmployeesPage() {
  ...
  const load = useCallback(async (term?: string, showSpinner = false) => {
    ...
    try {
      const result = await fetchEmployees({ page: 1, pageSize: 20, search: term });
      setData(result);
    } catch {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const result = await fetchEmployees({ page: 1, pageSize: 20 });
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {(data?.data ?? []).map((employee) => (
                  <tr key={employee.id} className="border-b border-border/60 last:border-0 hover:bg-muted/40">
